...
public class SearchEvent {
    ...
    private void printMenu(String whatYouWant) throws IOException {
        ...
        switch (whatYouWant) {
            case "1":
                eventService.printListOfEvents(searchInListByEventName(App.listOfEvents));
                break;
            case "2":
                eventService.printListOfEvents(searchInListByOrganizerName(App.listOfEvents));
                break;
            case "3":
    ...
    private List<Event> searchInListByOrganizerName(List<Event> eventsList) throws IOException {
        ...
        for (int i = 0; i < eventsList.size(); i++) {
            if (eventsList.get(i).getName().toLowerCase().contains(organizerName.toLowerCase())) {
                listFound.add(eventsList.get(i));
            }
            if (listFound.isEmpty()) {
                System.out.println("Nie znaleziono żadnych wyników");
